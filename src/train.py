...
from model.whisper import FlaxWhisperForConditionalGeneration
...
def main(strategy):
    ...
    for step, batch in zip(pbar, stream.train_iter()):
        ...
        if eval_counter == 0:
            ...
            if worker_id == 0:
                wandb.log({"eval_loss": curr_loss, "eval_accuracy": curr_acc, "epoch": epoch, "seen_examples": seen_examples, "step": step})
