...
def get_learning_rate_fn(training_args):
    ...
    if _learning_rate_fn is None:
        ...
        decay_fn = optax.linear_schedule(init_value=training_args.learning_rate, end_value=0, transition_steps=training_args.num_train_steps - training_args.warmup_steps)
        _learning_rate_fn = optax.join_schedules(schedules=[warmup_fn, decay_fn], boundaries=[training_args.warmup_steps])
